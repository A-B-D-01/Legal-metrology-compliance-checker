...
export default function CheckCompliancePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (inputType === 'url') {
        ...
        payload = formData;
        // Content-Type omitted dynamically for FormData to let browser add boundary
      }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-extrabold text-white tracking-tight">Audit Engine</h1>
        <p className="text-zinc-400 text-sm mt-1">Run automated OCR label extraction, web scraping, and RAG risk evaluations.</p>
      </div>
      ...
      <div className="bg-zinc-900/90 backdrop-blur-2xl p-6 rounded-2xl border border-zinc-800 shadow-[0_0_50px_rgba(0,0,0,0.8)] relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {inputType === 'url' ? (
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-400 mb-2">Product Target URL</label>
              <input
                type="url"
                required
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-400 mb-2">Packaging Image or Legal PDF Document</label>
              <input
                type="file"
                accept="image/*,.pdf"
                required
                onChange={(e) => setFile(e.target.files[0])}
                className="w-full text-xs text-zinc-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:bg-zinc-800 file:text-cyan-400 hover:file:bg-zinc-700 cursor-pointer border border-zinc-800 rounded-xl bg-zinc-950 p-1"
              />
            </div>
          )}
      ...
      {/* Results View */}
      {result && (
        <div className="bg-zinc-900/90 backdrop-blur-2xl p-6 rounded-2xl border border-zinc-800 shadow-2xl space-y-4">
          <div className="flex justify-between items-center border-b border-zinc-800 pb-4">
            <h2 className="text-lg font-bold text-white tracking-wide">Audit Report</h2>
          ...
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-400 mb-2">Findings Summary</h3>
            <p className="text-zinc-300 text-sm whitespace-pre-wrap leading-relaxed">{result.summary}</p>
          </div>
        </div>
      )}
    </div>
